...
// Query untuk mengambil data peminjaman dengan filter
$where_clauses = [];
$params = [];
$types = '';

if (!empty($search)) {
    $where_clauses[] = "(p.kode_peminjaman LIKE ? OR p.nama_anggota LIKE ? OR pd.judul_buku LIKE ?)";
    $search_param = "%{$search}%";
    for ($i = 0; $i < 3; $i++) {
        $params[] = $search_param;
        $types .= 's';
...
$query = "SELECT SQL_CALC_FOUND_ROWS p.*, 
          GROUP_CONCAT(DISTINCT pd.judul_buku SEPARATOR ', ') as judul_buku_list
          FROM peminjaman p
...
$peminjaman_list = mysqli_fetch_all($result, MYSQLI_ASSOC);
    ...
    <div class="container-fluid py-4">
                ...
                <div class="stat-card">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h3 class="mb-0 fw-bold text-warning"><?= $stats['dipinjam'] ?></h3>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="stat-card">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h3 class="mb-0 fw-bold text-danger"><?= $stats['terlambat'] ?></h3>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="stat-card">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h3 class="mb-0 fw-bold text-success"><?= $stats['dikembalikan'] ?></h3>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span><i class="fas fa-list me-2"></i> Daftar Peminjaman</span>
                <span class="badge bg-primary">Total: <?= $total_data ?> peminjaman</span>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= htmlspecialchars($peminjaman['nama_anggota']) ?></td>
                                <td>
                                    <small class="text-muted">
                                        <?= htmlspecialchars(substr($peminjaman['judul_buku_list'] ?? '-', 0, 50)) ?>...
                                    </small>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <?= date('d/m/Y', strtotime($peminjaman['tanggal_harus_kembali'])) ?>
                                </td>
                                <td>
                                    <span class="badge bg-<?= getStatusBadgeColor($peminjaman['status']) ?>">
                                        <?= ucfirst($peminjaman['status']) ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <a href="detail.php?id=<?= $peminjaman['id'] ?>"
                                           class="btn btn-info" title="Lihat Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="edit.php?id=<?= $peminjaman['id'] ?>"
                                           class="btn btn-warning" title="Edit Peminjaman">
